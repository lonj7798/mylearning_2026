...
- LN-Super-SFT (49B) scores IFEval 81.9 (on) and 83.0 (off) against 92.1 for Llama-3.3-70B-Instruct, the model from which LN-Super was derived by architecture search and 40B tokens of knowledge distillation (§2.1-§2.2); the report states that "reasoning-focused SFT causes a noticeable drop in IFEval scores" and adds a dedicated IFEval RL run (§7.3). The released LN-Super, after that run, two iterations of online RPO (reward-aware preference optimization) RLHF against Llama-3.1-Nemotron-70B-Reward, and model merging, scores 89.2 and 89.0 (§6, §7.3, Table 4).
