...
**범위**: 2026-09 revision에서 read.md를 primary source 기준으로 다시 썼다. 아래는 기존 Q1–Q18의 kernel 중 정정된 사실을 포함한 항목과 정정 내용이다. 기존 entry는 수정하지 않았다. 기존 entry의 line reference와 "§n" 표기는 git commit 4a72e54 시점의 read.md를 가리킨다. 정정 전체 목록은 [[read]]의 "Corrections to the version you studied"에 있다.

- **Q1**: "V^T 중 valid sequence는 V^1~V^2 수준"이라는 비율은 어떤 source도 측정하지 않았다. Interpretation으로만 읽어야 한다.
- **Q2**: OASST user turn에 typo가 흔하다는 서술은 근거가 없고, OASST guideline은 prompter에게 typo를 피하라고 요구한다 ([[openassistant]] App. A). "Vicuna > Baize-only on noisy user input" 실증도 source가 없다.
- **Q3**: Baize의 111.5k dialogue는 Quora와 Stack Overflow seed(각 약 55k)에서만 나왔다. Alpaca 51,942개는 single-turn training data로 추가됐고, MedQuAD 46,867개는 별도 healthcare set이다. 비용은 약 $100이고, self-chat template tag는 `[Human]`/`[AI]`이며 (`[|Human|]`는 inference prompt), 평균 turn은 3.6–3.9이다. SDF는 ChatGPT가 4개 response에 1–100 점수를 매기고 best response로 새 LoRA를 학습하는 방식이며 DPO는 없다. "Baize-13B > Alpaca-13B 58% human-eval"은 논문에 없고, 보고된 값은 GPT-4 score (ChatGPT 대비) Alpaca-13B 76%, Baize-v2-13B 92%이다. 또한 CAMEL (arXiv 2023-03)은 Baize보다 먼저 나왔으므로 "Baize의 후속"이 아니다. 논문은 filtering을 보고하지 않는다 ([[baize-construction]]).
- **Q4**: UltraChat 논문은 topic당 subtopic 30–50개, Sector I opening question 약 500,000개, C4 piece 10,000개 × instruction 5개를 적는다. 1,100+ subtopic과 ~100k C4는 README 수치이다. Sector별 dialogue 수 (~600K/~400K/~500K)는 보고되지 않았고 총계는 1,468,352이다. User model에는 "various user personalities" 지시가 들어간다 ([[ultrachat-two-model-protocol]]).
- **Q5**: Writing type 20개의 실제 목록은 논문 Table 3 (Articles and Blog Posts, Legal Documents and Contracts, Screenplays 등)이며 Q5의 목록과 다르다. "~400K (~27%)"와 "Zephyr/Tülu/Starling이 §2를 inherit"은 source가 없다.
- **Q6**: UltraChat은 filter를 하나 명시한다. 과도하게 공손한 user 문장 ("Thank you", "Thanks", "You're welcome")을 제거한다 (§4.4). Downstream 표의 Zephyr/Tülu/smol-talk 처리 방식은 이 chapter의 source로 확인되지 않았다.
- **Q7**: CAMEL AI Society는 50 × 50 role pair × task 10개 = 25,000 conversation이며, "50×50×20", "~1M", "$5-10K"는 틀렸다. Turn 분포 (easy 15%, median 6-8, cap 20)는 보고되지 않았고, 종료 조건은 5가지 (3 round 무지시, role reversal, end token, token limit, 40 message)이며 AI Society는 주로 assistant role reversal로 끝난다. "As an accountant" leak, token leak, Astronaut × Bartender는 논문에 없다. AgentInstruct와 APIGen-MT는 CAMEL을 인용할 뿐 Inception Prompt를 재사용한다고 서술되지 않는다.
- **Q8**: "50 role → 5 effective persona family", "10:1 collapse", "1M ≈ 100K distinct"는 측정값이 아니라 가설이다 (Open question). "§6 sibling variance (synthetic ≈ 0, OASST high)" 정량 증거는 존재하지 않는다. CAMEL 원래 규모도 1M이 아니라 25,000이다.
- **Q9**: Information anchor vs style anchor framework 자체는 Interpretation으로 유효하다. 다만 근거로 인용한 "sibling-variance metric"은 존재하지 않고, "APIGen-MT가 CAMEL보다 user-side diversity가 높다"는 source가 없다.
- **Q10**: SODA는 Atomic10x 크기를 적지 않는다 (~10M은 근거 없음). 1.5M triple은 x-relation 6개 (xAttr 18%, xEffect 17%, xIntent 23%, xNeed 7%, xReact 25%, xWant 11%)에서 나왔다. Narrative는 2–3 문장, filter는 4–20 turn 범위, 평균 7.6 turn과 utterance length 16.1 (unit 미기재), 비용은 triple 1개당 약 $0.02이다. COSMO-3B와 BlenderBot-3B는 둘 다 3B이며 ("1/3 parameter" 아님), out-of-domain human eval은 DailyDialog에서만 했다 (overall 72% vs 28%). Lineage "Self-Instruct → SODA → Prosocial-Dialog"는 틀렸다. ProsocialDialog (2022-05)가 SODA (2022-12)보다 먼저이고 Self-Instruct는 이 group의 논문이 아니다. ProsocialDialog의 unique RoT는 160,295개이고, "CANARY-400M 89% vs 32%"와 "Constitutional AI 직접 ancestor"는 근거가 없다. Canary는 T5-large safety/RoT generator이고 dialogue agent는 Prost (2.7B)이다. 4-pipeline 표의 "OOD generalization weak/med/strong"과 "σ_synth² 감소"는 측정되지 않았고, CAMEL도 Table 2에서 cross-domain transfer를 보고한다 ([[soda-commonsense-grounding]], [[prosocial-dialog]], [[camel]]).
- **Q11**: ProsocialDialog의 L1은 rule-of-thumb가 아니라 Social Chemistry, ETHICS, SBIC의 situation이고, GPT-3가 problematic utterance를 쓰고 crowdworker가 RoT와 feedback을 쓴다. ShareGPT는 2023년 초에 이미 존재했다 (Baize §3이 언급). Persona Hub (2024-06)와 WildChat (2024-05)만 CAMEL 이후이다.
- **Q12**: "ConceptNet → ATOMIC (24K) → ATOMIC-2020 (1.33M) → Atomic 10X (10M)" 경로와 "Atomic 10X ~5-10% noise"는 이 library의 source로 확인되지 않았다. SODA의 실제 filter는 erroneous pattern (반복, speaker prefix 누락; 6.3%), 4–20 turn (5.7%), 3인 이상 speaker (11.3%), non-human speaker (5.6%), Canary needs-intervention (4.3%), Rewire API (~1%), GPT-3.5 head-event check이다. SODA는 SmolTalk (Table 9)에도 Tülu 3 SFT mix에도 없다. Part B의 "응답률 ~1-3%"와 "polite refuse 60% / hostile 20% ..." 분포는 가상 수치이다.
- **Q13, Q14**: AgentInstruct의 suggester agent는 task를 더 복잡하거나 unsolvable하게 바꾸는 역할이며, implausible role pair를 사전에 reject하는 "task suggester"가 아니다 ([[agentinstruct]] §2). APIGen-MT/ToolACE의 filter triple 표는 확인되지 않았다. Q14의 "85% retirees hang up in 30s"는 예시용 가상 수치이다. Scenario-first와 cross-compatibility filter 설계 자체는 course proposal (Interpretation)로 유효하다.
- **Q15**: OASST1은 assistant message 69,614개, complete tree 10,968개 (전체 66,497), volunteer 13,500명 이상이다. 현재 collection setting은 tree depth 최대 5이다. "~50K+ preference pair"와 "10+ turn path <100개"는 보고되지 않았다. Reward model이 sibling ranking으로 학습된 것은 맞다 (App. H).
- **Q16**: "2× unique vocab, 3× persona-consistent follow-up, +5-10 IFEval"은 primary source가 없다. Persona Hub persona는 web bio/LinkedIn crawl이 아니라 RedPajama v2 web text에서 LLM이 추론한 것이고, dedup 후 1,015,863,523개이다. 비용 추정 ($50, 1주 등)은 source가 없다. Tülu 3에서 persona data를 빼면 IFEval이 72.8 → 53.6으로 떨어지지만 unseen IFEval-OOD는 17.6 → 18.0으로 거의 변하지 않는다.
- **Q17**: "Tülu-3, Qwen-2.5는 persona-conditioned gen", "Anthropic internal per-persona pipeline", "Strategy A로 IFEval +5-10", "Tülu-3/Smol-Talk가 stratified mix 구조"는 source로 확인되지 않았다. Strategy A vs B의 논리 (content-label mismatch)는 Interpretation으로 유효하다.
- **Q18**: SmolTalk에서 multi-turn이 3%라는 서술은 틀렸다. 가장 큰 구성요소 MagPie-Ultra (431k / 약 1.1M)가 three-turn conversation으로 생성됐고 Everyday-Conversations (2.38k)도 multi-turn이므로 최소 39.4%이다 (derived). "Real anchor 5-15%"와 "system-prompt IFEval -5~10"은 source가 없다. 참고로 Tülu 3 8B SFT의 WildChat 비중은 100,000 / 939,344 = 10.6%이다. Length를 Tier 3로 둔 판단은 짧은 chat에만 해당한다. Lost in Conversation에서 sharded multi-turn 평균 성능은 39% 떨어지고 unreliability는 112% 오른다. 또한 1.7B SFT에서 MagPie-Pro-MT가 single-turn MagPie-Pro보다 MT-Bench 5.40 vs 4.31로 높았다 ([[modern-mix-composition]], [[lost-in-multi-turn]]).
