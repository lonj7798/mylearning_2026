...
- Table 1 prints seven ARC-Challenge scores for Llama2-7B, one per reference plus OLMES: 53.1 (HF Open LLM Leaderboard), 45.9 (Llama 2 paper), 43.2 (Mistral 7B paper), 45.9 (Falcon paper), 48.5 (OLMo paper), 53.7 (Llama 3 model card, multiple-choice formulation), 54.2 (OLMES). Lowest to highest is 11.0 points for one model on one task.
- The column header of Table 1 records the setup behind each column: number of shots (25, 0, 0, 0, 0, 25, 5), whether the shots are curated (only OLMES), formulation (CF or MCF), and normalization (char, char, unknown, char, pmi, none, none/pmi).
