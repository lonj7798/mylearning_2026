...
**배경**: read.md가 2026-09 generality revision으로 새로 작성되었다. 아래 Qn의 kernel에는 이제 정정된 사실이 들어 있다. 괄호의 번호는 새 read.md "Corrections to the version you studied"의 항목 번호이다. Q1–Q7에 있는 section·line reference(예: "§8", "chapter §6", "§9 #4")는 git commit 4a72e54 시점의 read.md를 가리킨다.

- **Q1**: APIGen에는 layer별 제거 ablation table(88.24 / 82.2 / 77.3 / 70.1)이 없다. Fig. 5는 탈락한 data를 training에 다시 넣는 add-back 실험이고, xLAM-7B(FC)는 −4.06(semantic 탈락분)과 −5.94(execution 탈락분), xLAM-1B(FC)는 −9.59와 −12.17이다. Format arm은 없으므로 "Format-only ≈ 70 = Glaive ceiling"과 "execution이 가장 큰 single-layer lift"는 근거가 없다 (1). Generator는 DeepSeek-V2-Chat, DeepSeek-Coder-33B-Inst, Mixtral-8x22B-Inst, Mixtral-8x7B-Inst이고, semantic checker model은 명시되지 않았으며, MinHash dedup, k=1–3, 5초 timeout, type 비율 40/25/20/15, per-call cost는 보고되지 않았다 (4–5). Pass rate는 generator에 따라 34.42%–84.15%이고 "75% → 90% → 88%, 총 40% rejection" cascade는 source에 없다 (3). 3,673은 3,539 REST API + 134 Python function이다 (6). xLAM-7B(FC)의 base는 DeepSeek-Coder-7B-instruct-v1.5이고 BFCL 85.65(6위)이다 (2). "ToolLLaMA ~15% vs xLAM <3% hallucination"은 source에 없다. xLAM-2에는 DPO(β=0.1) stage가 보고되지 않았다 (7). License는 CC BY 4.0이다.
- **Q2**: Alignment validation의 committee majority vote가 Stage 2이고, Stage 3는 score threshold와 feedback 재생성이다 (23). Reverse task recombination 후에는 policy check를 다시 돌리고 Stage 2부터 재검증한다(§4.1.3). "~70% success"는 Phase 1 blueprint 성공률(feedback 있을 때 70%, 없을 때 28%)이고, Phase 2 trajectory 성공률은 67%이다 (22). gpt-4o와 DeepSeek V3는 generation, validation, interplay에 쓰였고 committee 구성 model은 보고되지 않았다. xLAM-2-70B의 pass^4 = 39.4는 text·table에 없고, pass^k는 BFCL이 아니라 τ-bench의 metric이다 (20–21). 저자가 쓴 limitation은 simulated user의 stochasticity, multi-stage validation overhead, 다른 domain과 RL 확장이다(§6).
- **Q3**: Paper는 simulated user가 instruction q와 persona를 받고 "underlying environment와 available API를 모른다"고 쓴다(§3.2.2). 따라서 "user-LLM이 ground_truth_actions를 본다(Setup C)"는 paper 기술과 맞지 않고, Setup A/B/C의 success rate(~100%, <50%, ~70%)는 source에 없다. 70%는 Phase 1 수치이다 (22). BFCL multi-turn은 turn마다 state-based check와 response-based check로 채점하며, LLM user simulator로 평가한다는 기술은 없다([[bfcl]] §4.4). 정보 비대칭에 대한 분석은 source 수치가 아니라 learner의 해석이다.
- **Q4**: "chapter §9 #4 directive (5–15% OASST/WildChat slice)"는 이 chapter의 source에 없다. APIGen-MT의 training data는 5K trajectory만이 아니라 APIGen data와 다른 agentic data를 섞은 mixture이다 (25). "xLAM-2 staged recipe의 60K SFT backbone"은 보고되지 않았다 (7). "BFCL pass^1 vs pass^k"는 τ-bench의 pass^k로 고쳐 읽는다 (20). APIGen-MT가 τ-bench와 같은 domain에서 생성하고 평가한다는 점(§4.3)은 distribution overfitting 주장과 관련된 확인된 사실이다.
- **Q5**: ToolACE에는 5 difficulty level, 30/25/20/15/10 target mix, forced resampling loop, "naive generation 시 70% simple collapse"가 없다. Complexity는 학습될 model 자신의 loss(Eq. 1)이고, user agent에게 더 어렵거나 쉬운 query를 쓰게 한다. Dialog type은 single, parallel, dependent, non-tool-use의 네 가지이다 (12–13, 34). Ablation −4.3 / −3.1 / −5.2 / −2.8은 틀리다. Non-live overall은 verification 없음 89.59, rule만 90.47, 두 layer 91.41(Fig. 3)이고 complexity subset은 90.47 / 90.71 / 89.65(Fig. 4)이다 (16). 91.41은 non-live ablation 점수이고, BFCL-v3 leaderboard에서는 ToolACE-8B 59.22 vs xLAM-7b-fc-r 51.45이다 (15). Clarification 요청은 assistant action space에 포함되고(§2.2.1), non-tool-use dialog를 빼면 irrelevance가 86.42 → 6.99로 떨어진다(Table 7).
- **Q6**: Rule layer는 call을 실행하지 않고 name, required parameter, regular-expression format으로 검사한다. "Partial execution ~10%"는 source에 없다 (11). Model layer는 3-way GPT-4 verdict가 아니라 hallucination detection, consistency validation, tool response check를 각각의 LLM agent가 수행하고, 결과는 human expert가 감독한다 (14). "~40% rejection → 11,300 dialogs"는 보고되지 않았다 (14). Generator와 judge model 이름은 명시되지 않았고, "GPT-4가 generation과 judging을 모두 한다"는 paper-acknowledged limitation이 아니다. App. H의 limitation은 complexity evaluation 비용·sampling bias와 general capability이다.
- **Q7**: BFCL single-turn category는 Simple, Multiple, Parallel, Parallel Multiple, Irrelevance, Relevance이고 Multi-Step category는 없다 (17). AST matching은 kwargs 정렬이나 `1.0 ≡ 1` 규칙이 아니라 parameter 값이 허용 답 집합에 있는지 보며, Python에서만 float 자리에 int를 허용한다 (18). Crowd-sourced category는 2,251 entry(+1,500 아님)이고, V4는 SWE-Gym이 아니라 web search, memory, format sensitivity를 추가한다 (19). pass^k는 τ-bench metric이다 (20). "ToolACE: training against the eval, not coincidentally matching" 인용은 paper에 없다. Granite의 seven task는 BFCL category와 다르다(Nested Function Calling, Function Chaining, Parallel Functions, Function Name Detection, Parameter-Value Pair Detection, Next-Best Function, Response Generation) (30). Hammer의 irrelevance data는 30%가 아니라 7,500개(약 11%)이다 (27). APIGen의 네 query style이 BFCL category에서 왔다는 점은 맞다(§3.3). "Benchmark가 data spec을 정한다"는 측정 결과가 아니라 해석이다 (34).
